...
# ════════════════════════════════════════════════════════
# Input Schemas (inline for simplicity in the admin panel)
# ════════════════════════════════════════════════════════
...
class TestCreate(BaseModel):
    course_id: int
    title: str
    test_type: str  # Unit/Practice/Pre-CAT/Mock/Revision/Final


class QuestionCreate(BaseModel):
    test_id: int
    question_text: str
    question_type: str  # MCQ / TrueFalse / ShortAnswer
    options: Optional[str] = None  # JSON string for MCQ
    correct_answer: str
...
async def upload_document(
    ...
    """Save PDF locally and upload to ChatPDF to make it queryable by the AI Advisor."""
    ...
    # 3. Upload to ChatPDF and get source_id
    ...
    if source_id:
        ...
        return {
            ...
            "doc_type": doc_type,
            "chatpdf_source_id": source_id,
            "status": "uploaded_to_chatpdf",
        }
    else:
        # Saved locally but ChatPDF key missing or upload failed
        return {
            "document_id": doc.id,
            "filename": file.filename,
            "doc_type": doc_type,
            "chatpdf_source_id": None,
            "status": "saved_locally — add CHATPDF_API_KEY to .env to enable AI querying",
        }


@router.delete("/documents/{document_id}")
def delete_document(document_id: int, db: Session = Depends(get_db)):
    """Remove document from DB and from ChatPDF."""
    doc = db.query(Document).filter(Document.id == document_id).first()
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")

    # Remove from ChatPDF if it was uploaded
    ...
    return [
        {
            ...
            "chatpdf_source_id": d.chatpdf_source_id,
...
# ════════════════════════════════════════════════════════
# Tests — Admin overview (admin/staff auth)
# ════════════════════════════════════════════════════════
...
def tests_overview(
    ...
    """All tests with attempt counts and avg scores."""
    tests = db.query(Test).all()
    result = []
    for t in tests:
        attempts = db.query(TestAttempt).filter(TestAttempt.test_id == t.id).count()
        results = db.query(TestResult).join(
            TestAttempt, TestResult.attempt_id == TestAttempt.id
        ).filter(TestAttempt.test_id == t.id).all()
        avg = round(sum(r.percentage for r in results) / len(results), 1) if results else 0
        result.append({
            "id": t.id, "title": t.title, "test_type": t.test_type,
            "course_id": t.course_id, "total_attempts": attempts, "avg_score": avg,
        })
    return result


@router.get("/test-attempts")
def all_test_attempts(
    skip: int = 0, limit: int = 50,
    db: Session = Depends(get_db),
    admin: AdminUser = Depends(get_current_admin)
):
    """All student test attempts with results (filtered by dept if staff)."""
    ...
    if admin.role == "staff" and admin.department:
        ...
        if dept:
            ...
            return [] # Staff department not found in DB
...
def all_submissions(
    ...
    """View all assignment submissions (filtered by dept if staff)."""
    ...
    for s in subs:
        ...
        out.append({
            ...
            "student": student.full_name if student else "?",
            "register_number": student.register_number if student else "?",
            "assignment_title": asgn.title if asgn else "?",
...
def grade_submission(
    ...
    """Admin/staff grades an assignment submission."""
...
def attendance_overview(
    ...
    """Attendance summary per student (filtered by dept if staff)."""
    q = db.query(Student)
    if admin.role == "staff" and admin.department:
        dept = db.query(Department).filter(Department.name == admin.department).first()
        if dept:
    ...
    for s in students:
        ...
        present = db.query(Attendance).filter(
            ...
            Attendance.status.in_(["Present", "OD"])
        ).count()
...
def generate_test_ai(course_id: int, topic: str, db: Session = Depends(get_db), admin: AdminUser = Depends(get_current_admin)):
    title = f"AI Test: {topic[:20]}"
    new_test = Test(title=title, test_type="Practice", course_id=course_id)
    db.add(new_test)
    db.commit()
...
def generate_assignment_ai(course_id: int, topic: str, db: Session = Depends(get_db), admin: AdminUser = Depends(get_current_admin)):
    ...
    try:
        content = qwen_generate(prompt, max_tokens=300)
...
def ai_evaluate_submission(sub_id: int, db: Session = Depends(get_db), admin: AdminUser = Depends(get_current_admin)):
    ...
    try:
        ...
        evaluation = f"AI Evaluation Failed (Network or API issue): {str(e)}"
